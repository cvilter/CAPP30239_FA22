...
d3.json('climate.json').then((data) => {      
    ...
    function updateChart(m) {
        ...
        .join(
            ...
            // changing the bar height and text based on new data
            // enter - putting it on the page. update - animation up. exit - animation down.
            update => {
            update.select("rect")
                .transition()
                .duration(750)
        ...
        let temp = d3.mean(data[m], d => d.average).toFixed(1); // to.Fixed(1) limits to 1 decimal place 
    ...
    d3.selectAll("select") // event listener. events can be done by user or browser
